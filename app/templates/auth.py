    ...
    try:
        ...
        if request.method == 'POST': 
            ...
            if attempt is not None:
                db.execute(
                    'SELECT * FROM user where id=?', (utils.U_CONFIRMED, attempt['id'])
                )
                db.execute(
                    'SELECT * FROM user where username=? AND password=? AND salt=? AND email=?' , (attempt['username'], attempt['password'], attempt['salt'], attempt['email'])
                )
                db.commit()
...
@bp.route('/register', methods=('GET', 'POST')) #se incluyen metodos
def register():
    try:
        if g.user:
            return redirect(url_for('inbox.show'))
        ...
        if request.method == 'GET':    
            username = request.form['username'] #pendiente verificar form
            ...
            if ((not utils.isUsernameValid(username)) or (not utils.isEmailValid(email))): #falta condicion + or + (not utils.isEmailValid(email))
                error =  'Email address invalid.'
                flash(error)
                return render_template('auth/register.html')
            
            if db.execute('Select * email from user ', (email,)).fetchone() is not None:
...
def confirm():
    try:
        ...
        if request.method == 'POST': 
            ...
            db = get_db()
            attempt = db.execute(
                'SELECT * FROM user WHERE id=?', (authid, utils.F_ACTIVE) #tabla authid?
            ).fetchone()
            ...
            if attempt is not None:
                ...
                hashP = generate_password_hash(password + salt)   
                db.execute(
                    'SELECT * FROM user WHERE contraseña=? AND salt=?', (hashP, salt, attempt['userid'])
                )
                db.commit()
                return redirect(url_for('auth.login'))
            else:
        ...
        return render_template('auth.login') #revisar template
    except:
        return render_template('auth/forgot.html')
...
def change():
    try:
        ...
        if request.method == 'POST': 
            ...
            db = get_db()
            attempt = db.execute(
               'SELECT * FROM user WHERE id=?', (number, utils.F_ACTIVE)
            ).fetchone()
...
def forgot():
    try:
        ...
        if request.method == 'POST':
            ...
            if ((not utils.isUsernameValid(user)) or (not utils.isEmailValid(email))):
                error = 'Email Address Invalid'
                flash(error)
                return render_template('auth/forgot.html')
            ...
            if user is not None:
                ...
                db.execute(
                    'SELECT * FROM user WHERE id=? AND ?????=??? AND ????=??',
...
def login():
    try:
        ...
        if request.method == 'POST':
            ...
            error = None
            user = db.execute(
                'SELECT * FROM user WHERE username ='+ username + 'and password =' + password, (username, password)
            ).fetchone()
